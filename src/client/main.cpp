...
int main(int argc, char **argv) {
    ...
    while (true) {
        ...
        switch (choice) {
            case 1: userType = "student"; goto login;
            case 2: userType = "teacher"; goto login;
            case 3: userType = "admin"; goto login;
            case 4: userType = "student"; goto reg;
            case 5: userType = "teacher"; goto reg;
            case 6: userType = "admin"; goto reg;
            case 0: close(clientfd); sem_destroy(&rwsem); exit(0);
            default: cerr << "无效选项!" << endl; continue;
        }
    ...
    login: {
        ...
        js["msgid"] = 
            (userType == "student") ? STU_LOGIN_MSG :
            (userType == "teacher") ? TEA_LOGIN_MSG : AD_LOGIN_MSG;
        js["id"] = id;
    ...
    reg: {
        ...
        js["msgid"] = 
            (userType == "student") ? STU_REG_MSG :
            (userType == "teacher") ? TEA_REG_MSG : AD_REG_MSG;
        js["name"] = name;
        js["password"] = password;
    ...
    while (true) {
        ...
        if (msgid == STU_LOGIN_MSG_ACK || msgid == TEA_LOGIN_MSG_ACK || msgid == AD_LOGIN_MSG_ACK) {
            if (js["errno"] == 0) {
                ...
                g_currentUser.type = 
                    (msgid == STU_LOGIN_MSG_ACK) ? "student" :
                    (msgid == TEA_LOGIN_MSG_ACK) ? "teacher" : "admin";
